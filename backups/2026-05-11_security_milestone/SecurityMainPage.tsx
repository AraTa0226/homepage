...
const MegaMenu = ({ show, categories, theme, onClose, navigate, handleMenuClick, securityData, findSlugByFlexibleName }: any) => {
    return (
        <AnimatePresence>
            {show && (
                <motion.div
                            ...
                            {categories.map((cat: any) => (
                                <div key={cat.id} className="flex flex-col gap-4">
                                    <div
                                        ...
                                        className={`flex flex-col gap-1 border-b border-emerald-500/10 pb-3 group/header ${(cat.groups || ['security_car', 'security_options', 'maintenance', 'security_full'].includes(cat.id)) ? 'cursor-default' : 'cursor-pointer'}`}
                                    >
...
const SecurityMainPage: React.FC<SecurityMainPageProps> = ({
    ...
    const { securityHeroAlert: heroAlert, securityRecruitment: recruitment, securityEvents, securityKnowledge, securityData, findSlugByFlexibleName } = usePrices();
    const [isLineupExpanded, setIsLineupExpanded] = useState(false);

    // Dynamic Menu Category Integration
    const securityCategories = useMemo(() => {
        const categories = [...(securityData?.menu?.categories || [])];
        const maintenanceIdx = categories.findIndex(cat => cat.id === 'maintenance');
        
        if (maintenanceIdx !== -1 && securityKnowledge?.theftMethods?.length > 0) {
            const dynamicItems = securityKnowledge.theftMethods.slice(0, 3).map((m: any) => m.title);
            // Keep fixed items like FAQ and Diagnostic Service
            const fixedItems = ["セキュリティー診断サービス", "よくあるご質問 (FAQ)"];
            
            // Add "View More" if there are more than 3 items
            const viewMore = securityKnowledge.theftMethods.length > 3 
                ? ["過去の盗難手口をもっと見る"] 
                : [];

            categories[maintenanceIdx] = {
                ...categories[maintenanceIdx],
                items: [...dynamicItems, ...viewMore, ...fixedItems]
            };
        }
        return categories;
    }, [securityData, securityKnowledge]);

    // Local SEO & Meta Tags
    React.useEffect(() => {
        if (!securityData?.home) return;
        const { seo } = securityData.home;
        document.title = seo?.title || "福岡市・大野城のカーセキュリティ専門店 ANG";
        ...
        <div className={`min-h-screen bg-white text-gray-900 font-sans selection:bg-emerald-500/30 selection:text-emerald-900`}>
                    ...
                    <nav className="hidden lg:flex items-center gap-6 xl:gap-8 text-sm font-bold uppercase tracking-widest shrink-0">
                        ...
                        >
                            ...
                            <MegaMenu
                                ...
                                findSlugByFlexibleName={findSlugByFlexibleName}
                            />
                        </div>
            ...
            <section id="services" className="py-32 px-4 bg-white relative">
                ...
                <div className="max-w-7xl mx-auto">
                    ...
                    <VaultGrid
                        categories={securityCategories.filter((cat: any) => cat.id !== 'mobile_solution')}
                        theme={theme}
            ...
            <section id="partners" className="py-32 bg-white relative overflow-hidden">
                <div className="max-w-7xl mx-auto px-4 relative z-10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
                        {(securityData.home?.partners || []).map((partner: any, idx: number) => (
                            <motion.a
                                key={idx}
                                href={partner.url}
                                target="_blank"
                                    ...
                                    {securityEvents.filter(e => e.status !== 'draft').length > 0 ? (
                                        <ul className="space-y-3">
                                            {securityEvents.filter(e => e.status !== 'draft').slice(0, 3).map(event => (
                                                <li key={event.id}>
                                                    <button 
                                                        onClick={() => navigate(`/security/news/${event.slug}`)}
                                                                                        ...
                                                                                        <div key={gIdx} className="space-y-3">
                                                                                            ...
                                                                                            <div className="grid grid-cols-2 gap-x-4 gap-y-2.5">
                                                                                                {group.items.map((item: string, iIdx: number) => {
                                                                                                    const slug = findSlugByFlexibleName(item);
                                                                                                    const vehicleData = slug ? (securityData.vehicles || {})[slug] : null;
                                                                                                    const displayName = vehicleData?.name || item;
