...
const cursorHistoryDir = path.join(appData, 'Cursor', 'User', 'History');

console.log('Searching Cursor history in:', cursorHistoryDir);
...
const allFiles = walk(cursorHistoryDir);
...
if (matches.length > 0) {
  fs.copyFileSync(matches[0].file, 'scratch/recovered_dashboard_cursor.tsx');
  console.log('Copied latest to scratch/recovered_dashboard_cursor.tsx');
}
